/** 87. Scramble String(leetcode) / 430. Scramble String(lintcode)
 ...
 *
 *      Example:1) Input: s1 = "great", s2 = "rgeat" ; Output: true
 *              2) Input: s1 = "abcde", s2 = "caebd" ; Output: false
 */
 
 //Method 1: recursive(递归) —— exponential complexity
  public boolean isScramble(String s1, String s2) {
    if (s1.equals(s2)) return true; 

    int[] letters = new int[256]; //8位ascii码共256个
    for (int i = 0; i<s1.length(); i++) {
        letters[s1.charAt(i)]++;
        letters[s2.charAt(i)]--;
    }
    for (int i = 0; i < 256; i++) if (letters[i]!=0) return false; //提前筛出两个字符串有不同字母或者长度不同的情况
 ...
 /* Method 2: DP - [idea and code from leetcode]
